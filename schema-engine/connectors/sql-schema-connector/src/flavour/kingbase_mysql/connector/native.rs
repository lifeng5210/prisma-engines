...
use quaint::{
    connector::{KingbaseMysql, KingbaseMysqlUrl},
    prelude::{NativeConnectionInfo, Queryable},
};
use schema_connector::{ConnectorError, ConnectorResult};
use sql_schema_describer::{DescriberErrorKind, SqlSchema, SqlSchemaDescriberBackend, kingbase_mysql::Circumstances};
use url::Url;
use user_facing_errors::schema_engine::{ApplyMigrationError, DatabaseSchemaInconsistent};

pub(super) struct Connection {
    client: KingbaseMysql,
    url: KingbaseMysqlUrl,
}

impl Connection {
    pub(super) async fn new(url: Url) -> ConnectorResult<Self> {
        let url = KingbaseMysqlUrl::new(url).map_err(ConnectorError::url_parse_error)?;
        let connection = KingbaseMysql::new(url.clone()).await.map_err(|error| {
            quaint_error_to_connector_error(error, Some(&NativeConnectionInfo::KingbaseMysql(url.clone())))
        })?;
    ...
    pub(super) async fn describe_schema(&mut self, params: &Params) -> ConnectorResult<SqlSchema> {
        let schema = sql_schema_describer::kingbase_mysql::SqlSchemaDescriber::new(
            &self.client,
            Circumstances::CheckConstraints.into(),
        )
        .describe(&[params.schema_name()])
        .await
    ...
    fn map_error(&self, error: quaint::error::Error) -> ConnectorError {
        let connection_info = NativeConnectionInfo::KingbaseMysql(self.url.clone());
