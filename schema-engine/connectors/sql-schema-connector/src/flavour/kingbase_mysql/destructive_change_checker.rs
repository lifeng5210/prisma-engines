...
/// Kingbase MySQL uses the MySQL-compatible destructive-change algorithm, but
/// diagnostics must render types through the Kingbase PSL connector.
#[derive(Debug, Default)]
pub(crate) struct KingbaseMysqlDestructiveChangeCheckerFlavour;

impl KingbaseMysqlDestructiveChangeCheckerFlavour {
    fn datamodel_connector(&self) -> &dyn psl::datamodel_connector::Connector {
        psl::builtin_connectors::KINGBASE_MYSQL
    }
}

impl DestructiveChangeCheckerFlavour for KingbaseMysqlDestructiveChangeCheckerFlavour {
