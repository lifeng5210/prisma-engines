...
#[test_connector(tags(KingbaseMysql), exclude(Vitess))]
    ...
    api.barrel()
        .execute(move |migration| {
            migration.create_table("x", move |t| {
                t.add_column("id", types::primary());
                t.add_column("y", types::integer().nullable(false));
                t.add_constraint("unique_y_id", types::unique_constraint(["id", "y"]));
            });
...
async fn relations_and_referential_actions_are_introspected(api: &mut TestApi) -> TestResult {
    api.raw_cmd(indoc! {r#"
        CREATE TABLE City (
        ...
        CREATE INDEX UserCity_city_fkey_idx ON UserCity (city_id, city_code);
    "#})
        .await;
    ...
    let expected = expect![[r#"
        ...
        model UserCity {
          ...
          City      City?   @relation(fields: [city_id, city_code], references: [id, code], onDelete: Cascade, onUpdate: NoAction, map: "UserCity_city_fkey")

          @@index([city_id, city_code], map: "UserCity_city_fkey_idx")
        }
    "#]];
    for _ in 0..4 {
        expected.assert_eq(&api.introspect_dml().await?);
...
#[test_connector(tags(KingbaseMysql))]
async fn required_and_optional_one_to_many_relations_are_introspected(api: &mut TestApi) -> TestResult {
    api.raw_cmd(indoc! {r#"
        CREATE TABLE `User` (
            id INT NOT NULL AUTO_INCREMENT PRIMARY KEY
        );

        CREATE TABLE RequiredPost (
            id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
            CONSTRAINT RequiredPost_user_fkey
                FOREIGN KEY (user_id) REFERENCES `User` (id)
                ON DELETE RESTRICT ON UPDATE CASCADE
        );

        CREATE INDEX RequiredPost_user_idx ON RequiredPost (user_id);

        CREATE TABLE OptionalPost (
            id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
            user_id INT,
            CONSTRAINT OptionalPost_user_fkey
                FOREIGN KEY (user_id) REFERENCES `User` (id)
                ON DELETE SET NULL ON UPDATE CASCADE
        );

        CREATE INDEX OptionalPost_user_idx ON OptionalPost (user_id);
    "#})
        .await;
    ...
    let expected = expect![[r#"
        model OptionalPost {
          id      Int   @id @default(autoincrement())
          user_id Int?
          User    User? @relation(fields: [user_id], references: [id], map: "OptionalPost_user_fkey")

          @@index([user_id], map: "OptionalPost_user_idx")
        }

        model RequiredPost {
          id      Int  @id @default(autoincrement())
          user_id Int
          User    User @relation(fields: [user_id], references: [id], map: "RequiredPost_user_fkey")

          @@index([user_id], map: "RequiredPost_user_idx")
        }
...
#[test_connector(tags(KingbaseMysql))]
async fn one_to_one_and_self_relations_are_introspected(api: &mut TestApi) -> TestResult {
    api.raw_cmd(indoc! {r#"
        CREATE TABLE `User` (
            id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
            manager_id INT,
            CONSTRAINT User_manager_fkey
                FOREIGN KEY (manager_id) REFERENCES `User` (id)
                ON DELETE SET NULL ON UPDATE CASCADE
        );

        CREATE UNIQUE INDEX User_manager_unique ON `User` (manager_id);

        CREATE TABLE Profile (
            id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
            CONSTRAINT Profile_user_fkey
                FOREIGN KEY (user_id) REFERENCES `User` (id)
                ON DELETE CASCADE ON UPDATE CASCADE
        );

        CREATE UNIQUE INDEX Profile_user_unique ON Profile (user_id);
    "#})
        .await;
    ...
    let expected = expect![[r#"
        model Profile {
          id      Int  @id @default(autoincrement())
          user_id Int  @unique(map: "Profile_user_unique")
          User    User @relation(fields: [user_id], references: [id], onDelete: Cascade, map: "Profile_user_fkey")
        }

        model User {
          id         Int      @id @default(autoincrement())
          manager_id Int?     @unique(map: "User_manager_unique")
          Profile    Profile?
          User       User?    @relation("UserToUser", fields: [manager_id], references: [id], map: "User_manager_fkey")
