...
#[test_connector(tags(KingbaseMysql), exclude(Vitess), preview_features("views"))]
...
#[test_connector(tags(KingbaseMysql), exclude(Vitess, Mariadb), preview_features("views"))]
async fn simple_view_from_two_tables(api: &mut TestApi) -> TestResult {
    ...
    let expected = expect![[r#"
        SELECT
          u.id,
          concat_text(
            VARIADIC ARRAY [(u.first_name)::text, ' '::text, (u.last_name)::text]
          ) AS name,
          p.introduction
        FROM
          (
...
#[test_connector(tags(KingbaseMysql), preview_features("views"))]
async fn views_are_introspected(api: &mut TestApi) -> TestResult {
    api.raw_cmd(indoc! {r#"
        ...
        CREATE VIEW source_view AS
            SELECT id, label FROM source_rows;
    "#})
        .await;
    ...
    let view_definition = expect![[r#"
        SELECT
          source_rows.id,
          source_rows.label
        FROM
          source_rows;"#]];
    api.expect_view_definition("source_view", &view_definition).await;
